        ...
        >
            <el-form ref="formRef" :model="formData" label-width="84px" :rules="formRules">
                ...
                <el-form-item label="来源[0->购物；1->管理员修改;2->活动]" prop="sourceTyoe">
                   <el-checkbox v-model="formData.sourceTyoe" :true-label="1" :false-label="0" placeholder="请选择来源[0->购物；1->管理员修改;2->活动]"></el-checkbox>
                </el-form-item>
            </el-form>
        </popup>
    </div>
...
const formRules = {
    ...
    sourceTyoe: [
        {
            required: true,
            message: '请选择来源[0->购物；1->管理员修改;2->活动]',
            trigger: ['blur']
        }
    ],
}
...
const handleSubmit = async () => {
    ...
    data.sourceTyoe = data.sourceTyoe.join(',')
...
const setFormData = async (data: Record<string, any>) => {
    for (const key in formData) {
        if (data[key] != null && data[key] != undefined) {
            ...
            //@ts-ignore
            formData.sourceTyoe = String(data.sourceTyoe).split(',')
        }
    }
}
